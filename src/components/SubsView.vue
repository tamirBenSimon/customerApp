<template>
  <div class='upload-order-dashbord'>
    ...
    <div class="controls-1">Sub-Batches Center</div>
    ...
    <el-table @select='checkboxChecked' @select-all='checkboxChecked' class='my-table' :data='tableData'>
      ...
      <template slot-scope="scope">
        <el-button
          @click.native.prevent="goToLabels(scope.$index, tableData)"
          type="text"
...
export default {
  ...
  props: ['subs'],
  methods: {
    goToLabels (index, rows) {
      console.log('in subview')
      console.log(rows[index])
      this.$emit('goToLabels', rows[index].SubId)
    },
